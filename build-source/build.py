...
(function () {
  ...
  btn.addEventListener("click", function (e) {
    e.preventDefault();
    var t = timeEl.value;
    if (!t) { showToast("Διάλεξε ώρα (12:00–15:30)."); timeEl.focus(); return; }
    var mins = parseInt(t.slice(0, 2), 10) * 60 + parseInt(t.slice(3, 5), 10);
    if (mins < 720 || mins > 930) { showToast("Η ώρα πρέπει να είναι μεταξύ 12:00 και 15:30."); timeEl.focus(); return; }
    var txt = buildText();
...
<div class="order-bar" id="orderBar" role="region" aria-label="Η παραγγελία σου">
  <div class="order-opts">
    ...
    <label class="time-field">Ώρα<input type="time" id="orderTime" min="12:00" max="15:30" step="300"></label>
  </div>
