...
CSS = """
  ...
  *{box-sizing:border-box;} html{-webkit-text-size-adjust:100%;}
  ...
  /* Τελευταίες μερίδες (από το «few» του soldout.json): ΧΩΡΙΣ αριθμό — απόφαση
     ιδιοκτήτη 15/9/2026, ο αριθμός αργεί 1–3' και θα εξέθετε το μαγαζί. Κάτω από
     το όνομα, όχι δίπλα στην τιμή: σε κινητό η γραμμή δεν χωράει και το «+». */
  .is-few .gr::after{content:"ΤΕΛΕΥΤΑΙΕΣ ΜΕΡΙΔΕΣ";display:block;margin-top:.2rem;
               font-size:.66rem;font-weight:800;letter-spacing:.08em;color:var(--sea);}
  ...
  /* Ποτέ ψηλότερη από την οθόνη: με ανοιχτά τα στοιχεία πελάτη είναι ~475px, και
     με το πληκτρολόγιο του κινητού η ορατή οθόνη πέφτει στα ~330px — το πάνω μέρος
     (Delivery/Παραλαβή, ώρα, ακόμη και η διεύθυνση) έβγαινε έξω χωρίς να κυλά.
     dvh = το ορατό ύψος, που μικραίνει με το πληκτρολόγιο· vh για παλιούς browser. */
  .order-bar{max-height:100vh;max-height:100dvh;overflow-y:auto;overscroll-behavior:contain;}
...
(function () {
  ...
  items.forEach(function (li) {
    li.querySelector(".q-plus").addEventListener("click", function () {
      ...
      // όριο από τις «τελευταίες μερίδες» — ο αριθμός δεν φαίνεται, μόνο φρενάρει
      var max = parseInt(li.getAttribute("data-max"), 10);
      if (max && qOf(li) >= max) {
        showToast("Έχουν μείνει λίγες μερίδες — δεν γίνεται περισσότερες.");
        return;
      }
  ...
  //
  // «few» = τελευταίες μερίδες: {κλειδί: 2 ή 1}. Ο πελάτης βλέπει μόνο την
  // ένδειξη· ο αριθμός γίνεται όριο στο «+». Στα συνοδευτικά δεν ισχύει (εκεί
  // επιλέγονται μέσα στο πιάτο) — το soldout.py δεν τα γράφει καν, αλλά και εδώ
  // αγνοούνται, για να μη μείνει μισή λειτουργία αν αλλάξει ποτέ εκείνο.
  var MENU_ISO = __ISO_JSON__;
  var soldSig = "";

  function applySoldout(list, few) {
    var sig = list.slice().sort().join(",") + "|" + JSON.stringify(few);
    if (sig === soldSig) return;
    soldSig = sig;
    var out = {}, removed = [], reduced = [];
    list.forEach(function (k) { out[k] = true; });
    sidesOut = {};
    items.forEach(function (li) {
      var key = li.getAttribute("data-key");
      var gone = !!out[key];
      var max = gone || key.indexOf("synodeytika:") === 0 ? 0 : (few[key] === 1 || few[key] === 2 ? few[key] : 0);
      li.classList.toggle("is-soldout", gone);
      li.classList.toggle("is-few", !!max);
      if (max) li.setAttribute("data-max", max); else li.removeAttribute("data-max");
      if (max && qOf(li) > max) { reduced.push(li.getAttribute("data-name")); setQty(li, max); }
      if (!gone) return;
    ...
    else if (reduced.length) showToast("Έμειναν λίγες μερίδες από: " + reduced.join(", ") + ". Η ποσότητα μειώθηκε.");
  ...
  function checkSoldout() {
    ...
    fetch("soldout.json?ts=" + Date.now(), {cache: "no-store"})
      ...
      .then(function (j) {
        ...
        var today = j.date_iso === MENU_ISO;
        applySoldout(today && Array.isArray(j.items) ? j.items : [],
                     today && j.few && typeof j.few === "object" ? j.few : {});
      })
