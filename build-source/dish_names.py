...
def resolve(token, rows, where="στο DAILY_MENU.xlsx"):
    ...
    # 2β. ίδιο όνομα σε άλλη πτώση ή αριθμό («μουσακά» -> «Μουσακάς»): ίδιες
    #     ρίζες, ίδια σειρά, ούτε λέξη παραπάνω. Αλλιώς το «μουσακά» έπεφτε στο
    #     βήμα 3 και ταίριαζε ΚΑΙ στον «Μουσακάς νηστίσιμος» — ασάφεια για ένα
    #     όνομα που είναι ολόκληρο γραμμένο.
    qs = [_stem_word(w) for w in query_words(token)]
    if qs:
        hit = [n for n in rows
               if [_stem_word(w) for w in words(name_of(n))] == qs]
        if len(hit) == 1:
            return hit[0], None
