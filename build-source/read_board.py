...
# Ημερομηνία σε οποιαδήποτε από τις μορφές που δέχεται το menu_date.
_DATEISH = re.compile(r"\d{1,2}\s*[/\-.]\s*\d{1,2}(?:\s*[/\-.]\s*\d{2,4})?")


def _date_only(line):
    """True αν η γραμμή δεν έχει τίποτε άλλο πέρα από ημερομηνία και τη μέρα.

    «Τρίτη 1/9» ναι· «12/8 σουβλάκια» όχι — εκεί το 12/8 είναι μέρος του πιάτου
    ή σκέτη σύμπτωση, και η γραμμή πρέπει να μείνει στη λίστα.
    """
    rest = dish_names.norm(_DATEISH.sub(" ", line))
    return not rest or any(rest == dish_names.norm(d) for d in GREEK_DAYS)
...
def read_text(body):
    ...
    # Πρώτη γραμμή που είναι ΜΟΝΟ ημερομηνία = δήλωση ημέρας.
    #
    # Δεν αρκεί «διαβάζεται ως ημερομηνία»: το «12/8 σουβλάκια» διαβαζόταν ως
    # 12 Αυγούστου, η γραμμή έφευγε, και το πιάτο εξαφανιζόταν ΣΙΩΠΗΛΑ — με
    # ημερομηνία περασμένη από πάνω. Ο παλιός όρος «χωρίς κόμμα» δεν το έπιανε:
    # μια γραμμή με ένα μόνο πιάτο δεν έχει κόμμα.
    day = None
    if lines and "," not in lines[0] and _date_only(lines[0]):
